...
import { UrlEntrySchema, SORTABLE_FIELDS, extractDomain, } from "@repo/shared";
export const urlsRouter = router({
    ...
    list: authedProcedure
        ...
        if (input.aiModel)
            where.aiModelMentioned = input.aiModel;
        if (input.sentiment)
    ...
    stats: authedProcedure.query(async ({ ctx }) => {
        ...
        const [totalUrls, uniqueDomains, avgVisibility, byDomainRaw, byModelRaw, bySentimentRaw, byDateRaw] = await Promise.all([
