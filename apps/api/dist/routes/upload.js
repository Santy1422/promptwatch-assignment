...
export function registerUploadRoute(fastify) {
    fastify.post("/api/upload", async (request, reply) => {
        // Validate API key
        const apiKeyValue = request.headers[API_KEY_HEADER];
        if (!apiKeyValue) {
        ...
        const { succeeded, failed } = await processBatches(validRows, (row, data) => prisma.urlEntry.upsert({
